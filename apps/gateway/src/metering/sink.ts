...
export class MeteringUsageSink implements UsageSink {
  ...
  #backend(env: unknown, tenantId?: string): MeteringBackend {
    ...
    // The unattributed (`tenant IS NULL`) backend keys under a platform-scoped
    // sentinel — after the Track A hard-cut that backend resolves to the
    // PLATFORM_DATA singleton, never the shared control projection.
    const key = tenantId === undefined || tenantId === "" ? "__platform__" : tenantId;
  ...
  async #deliverOnce(
    ...
    try {
      ...
      // Track A hard-cut: an UNATTRIBUTED settlement is settled DIRECTLY into the
      // PLATFORM_DATA singleton by `#backend` (`meteringDatabaseFrom`'s
      // undefined-tenant branch now resolves the platform object, never the shared
      // control projection), so there is no longer a separate control write to
      // shadow. The old dual-write leg here is gone with the control mirror.
      this.#outbox.delete(id);
   ...
   * either a tenant backend or the unattributed platform backend that
   * {@link #backend} memoizes on the env (an undefined tenant now resolves the
   * PLATFORM_DATA object — Track A hard-cut). `accumulate` is the one behavioural
   * difference: a tenant sweep re-runs the idempotent usage aggregate when a
   * binding resolver is configured, while an unattributed charge has no tenant
   * object to roll up into.
   */
  ...
  // `sweepPlatform` is REMOVED (Track A hard-cut). Crash-stranded UNATTRIBUTED
  // outbox rows now live in the PLATFORM_DATA object like every other row, so the
  // ordinary `sweep(rc, …, undefined)` path recovers them: `#sweepBackend` with an
  // undefined tenant resolves the platform backend through `#backend`
  // (`meteringDatabaseFrom`'s undefined-tenant branch → the platform object), no
  // longer a separate hand-built control backend.

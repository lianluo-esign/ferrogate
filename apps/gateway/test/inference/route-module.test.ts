...
describe("inferenceRouteModule", () => {
  ...
  it("claims exactly the 15 contract inference operation ids", () => {
    ...
    // parent's. #689's `getResponse` / `deleteResponse` take it to 14, and the
    // Gemini-native ingress (`geminiGenerateContent`,
    // POST /v1beta/models/{model}:generateContent) to 15, counted off the list
    // the module actually claims.
    expect(inferenceRouteModule().operationIds).toHaveLength(15);
  });

  it("registers all 15 on the contract-driven router", () => {

...
export function admissionFromEnv(
  ...
  return new McpAdmissionGate({
    ...
    quotas: d1QuotaPolicySource(db, undefined, quotaTenantPolicyDb(router)),
 ...
 * no router is bound (the tenant legs are then skipped and the limiter fails
 * open — control holds no mirror to read).
 *
 * Returns a per-tenant accessor for the caller tenant's OWN object whenever a
 * tenant router is bound — the tenant object is the SOLE authority now, so there
 * is no posture gate. `TenantDatabaseRouter.forTenant` already resolves the
 * handle for exactly the requested tenant, so `handle.tenantId` must equal it —
 * the same cross-tenant tripwire the spend legs arm, refusing rather than reading
 * one tenant's caps from another's object. Any resolution failure (a router that

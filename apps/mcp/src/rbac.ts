 ...
 * ## What FC-7 was
 *
 * `src/contract.ts:253` parsed `rbac_action` off the shared contract table into
 * every {@link ApiOperation} this Worker serves — and **nothing ever read it**.
 * `apps/agent-runtime` had the identical hole. So a role that denies an action
 * was enforced on `apps/gateway` and `apps/control-plane` and silently ignored
 * on the other two: "call the other endpoint", which is the shape both of this
 * project's shipped fleet defects had (the wave-16 admission bypass and the
 * wave-20 agent-upstream half-withdrawal).
 *
 ...
 * paths, which this Worker does not serve. It was a control that had been
 * *pre-armed* to fail: the first `rbac_action` on a data-plane operation would
 * have been unenforced on two of five Workers, with every per-Worker suite
 * green because every Worker was individually correct.
 *
 * That is why the fix is STRUCTURAL rather than a new gate on a named route.
 * `src/http.ts::authenticateRequest` consults this port for whatever the
 * MATCHED contract operation declares, so the day an `rbac_action` lands on
 * `POST /v1/mcp` the enforcement is already here and no one has to remember it.
 *
 ...
 * tenant_has_permission_result`, and the three steps are load-bearing:
 *
 *  1. **The permission must EXIST** (`permissions.key`). An action nobody
 *     declared is denied even if a role happens to name it — that is what stops
 *     a typo in a role's `permission_keys` from minting an entitlement.
 ...
 *  3. **A role grants when its `permission_keys` CONTAINS the action.** There
 *     is deliberately no wildcard: Rust compares `key == permission_key`, so
 *     `"*"` in a role is a permission literally named `*` and nothing else.
 *
 * Two departures, both narrowing and both matching `apps/gateway`:
 *
 *  - a **platform operator** is waved through before any query (every Rust call
 *    site is `if let CallerScope::Tenant(..) = auth.caller_scope()`);
 *  - a **storage failure** answers `allowed: "unavailable"` → 503
 *    `rbac_unavailable`, never a decision. An outage must not be
 *    indistinguishable from a policy answer, and fail-open here would make
 *    "flap the control plane" an authorization bypass.
 *
 * ## What this file deliberately does NOT do
 *
 * `apps/gateway` carries a second, VAR-backed authorizer
 * (`ConfiguredRbacAuthorizer` over `TENANT_RBAC_ACTIONS`) as an additive
 * bootstrap in front of the durable graph. It is not ported here. Rust has no
 * config-file RBAC at all, the var is declared only in `apps/gateway/
 * wrangler.toml`, and reading a var this Worker's deploy config does not
 * declare would fail `test/env-var-drift.test.ts` for a value no operator could
 * set. The consequence is stated rather than hidden: a grant that exists ONLY
 * in the gateway's var table is honoured there and denied here. That asymmetry
 * is in the DENY direction on this Worker, which is the safe half; the
 * authority both Workers share — the durable graph — decides identically, and
 * `apps/gateway/test/fleet-rbac-action.test.ts` drives one seeded control
 * database through both to prove it.
 *
 * ## No control database ⇒ 503, never an open door
 *
 * `env.DB` is the CONTROL database on this Worker (`wrangler.toml`,
 * `database_name = "ferrogate-control"`). With no binding there is no grant
 * graph to read, and an operation that DECLARES an `rbac_action` cannot be
 * authorized without one — so {@link UnboundRbacAuthorizer} answers
 * `unavailable` (503), the same posture `UnboundAuth` takes for credentials.
 * An operation with no `rbac_action` never reaches this port at all, so an
 * unbound deployment serves exactly what it served before.
 */
import {
 ...
 * it. `apps/agent-runtime/src/guardrails.ts` carries the same note.
 */
...
export class UnboundRbacAuthorizer implements RbacAuthorizerPort {
  ...
  async authorize(): Promise<RbacDecision> {
    return {
      allowed: "unavailable",
 ...
 *
 * `env.DB` bound ⇒ the durable walk. Absent ⇒ {@link UnboundRbacAuthorizer}.
 * Bound in the DEV posture too, for the provability reason every other port in
 * `src/ports.ts` is: the offline suite drives the deployed app there, so a gate
 * mounted anywhere else would be untestable over `SELF` — "implemented, tested,
 * never mounted" being the defect this project keeps repeating.
 */
export function rbacAuthorizerFromEnv(env: {
  DB?: unknown;
  TENANT_DATA?: unknown;
}): RbacAuthorizerPort {
  if (!isRbacDatabase(env.DB)) return new UnboundRbacAuthorizer();
  const namespace = env.TENANT_DATA as TenantDataNamespace | undefined;
  const tenantDatabases =
    namespace === undefined
      ? undefined
      : new DurableObjectTenantDatabaseRouter(namespace, env.DB as D1Database);
  return new D1RbacAuthorizer(env.DB, { tenantDatabases });
}

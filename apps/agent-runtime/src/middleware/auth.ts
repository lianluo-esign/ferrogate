...
/**
 * THE ADMISSION HALF, ported (`gateway/auth.rs::finalize_auth`).
 *
 * This block used to carry an open portability marker, and that marker named a
 * live control bypass rather than a fidelity gap. In the Rust tree `/v1/agent-runs`, `/v1/agent-jobs/**` and
 * `/v1/agents/**` were served by the SAME process as `/v1/chat/completions` and
 * went through the same `authenticate()` → `finalize_auth` chain. Splitting the
 * data plane across Workers moved only the CREDENTIAL half here, so a tenant
 * whose RPM cap and monthly budget stopped it dead on `/v1/chat/completions`
 * could submit unbounded agent jobs on the same key — and each job then spends
 * real provider money through the gateway on the run's behalf.
 *
 * `deps.admission.admit()` below is the missing half, in Rust's order:
 * `503 quota_resolution_unavailable`, `403 quota_scope_disabled`,
 * `429 monthly_budget_exceeded`, `429 wallet_balance_exhausted`,
 * `429 rate_limit_exceeded`, `503 governance_counter_unavailable`. See
 * `../admission/index.ts` for what is imported (the `@ferrogate/policy` quota
 * merge and counter-key namespacing, the `@ferrogate/storage` no-oversell
 * batch) rather than re-implemented, and for the ONE `wrangler.toml` binding the
 * integrate step adds so this Worker charges the GATEWAY's counter namespace
 ...
 * ladder (`tenancy_suspended`). Admission runs AFTER both, exactly as
 * `finalize_auth` does — a suspended tenant must not even reach quota/wallet
 * resolution, which is where spend is authorized.
 *
 * It runs after the SCOPE check too, which Rust does not do (Rust checks scopes
 * in the handler, downstream). The difference is deliberate and is the
 * tightening direction: a request that will be refused 403 `scope_denied`
 * anyway must not first spend a slot from the caller's RPM window, or a client
...
export async function bearerAuth(
  ...
  // returns the wallet holds for an admitted one; `contractAuth` releases them
...
export const contractAuth: MiddlewareHandler<AgentRuntimeEnv> = async (c, next) => {
  ...
  // Rust released a `WalletCreditReservation` when the guard DROPPED, so a
  // handler that threw, or a request that was refused downstream, still gave
  // its credits back. TypeScript has no destructor, so the `finally` IS the
  // release — and it must wrap `next()` rather than follow it, or a throwing
  // handler would strand the hold until its TTL expired.
  const grant = c.get("admissionGrant");
  if (grant === undefined) {
    await next();
    return;
  }

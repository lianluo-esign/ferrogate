 ...
 * another tenant's aggregate window, it would drain a victim's RPM budget and
 * deny them service (cross-tenant DoS).
 *
 * The invariant, from `ferrogate_policy::QuotaScopeSelector::counter_key` and
 * `ferrogate_gateway::auth::AuthContext::request_windows`:
 *
 ...
 * `api_key_id`. A tenant that mints a virtual key whose id is literally
 * `"tenant:victim"` therefore produces `"key:tenant:victim"` — structurally
 * unequal to the victim's `"tenant:victim"` aggregate window.
 *
 * **That derivation is NOT re-implemented here.** `QuotaScopeSelector.counterKey`
 * from `@ferrogate/policy` is CALLED, so there is exactly one copy of the rule
 * in the tree and this Worker cannot drift from `apps/gateway` — which matters
 * more here than anywhere, because the two Workers are supposed to share one
 * counter namespace (see `./counter.ts`). What this module adds is the second
 * half a port needs: a boundary check ({@link assertNamespacedCounterKey}) that
 * refuses to address a counter with any string that is not namespaced.
 */
import {
  type EffectiveQuota,
  type QuotaScopeKind,
 ...
 * window runs this first, so a raw `api_key_id` — or any other unnamespaced,
 ...
 * derivation goes through `QuotaScopeSelector.counterKey`. The guard exists
 * because "the derivation is correct today" is not a property a future call
 * site preserves on its own.
 */
 ...
 *
 *  1. the TOK-12 per-key `api_keys.request_limit_per_minute` on the credential
...
export function requestWindows(
  ...
  const add = (counterKey: string, limit: number): void => {
    const existing = windows.find((window) => window.counterKey === counterKey);
 ...
 * `wallet_reservations` on the bare tenant id (`organization_id`); namespaced to
 * `"tenant:{id}"` here for the same reason as everything else in this file.
 */

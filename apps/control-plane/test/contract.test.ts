...
describe("contract document", () => {
  it("is version 1 and carries all 306 operations", () => {
    expect(RAW.version).toBe(1);
    expect(RAW.operations).toHaveLength(EXPECTED_TOTAL_OPERATION_COUNT);
  });

  it("assigns exactly 238 operations to apps/control-plane", () => {
  ...
  it("declares exactly 45 route groups", () => {
    ...
    expect(new Set(RAW.route_patterns.map((pattern) => pattern.group)).size).toBe(45);
  });

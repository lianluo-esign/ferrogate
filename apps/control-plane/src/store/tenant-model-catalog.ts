...
function boolValue(value: number | boolean | null | undefined, fallback = false): boolean {
  if (value === null || value === undefined) return fallback;
  return value === true || value === 1;
}

function boolSql(value: boolean | null | undefined, fallback: number | null = 1): number | null {
  if (value === null) return null;
  if (value === undefined) return fallback;
  return value ? 1 : 0;
}

function providerCompatibility(kind: string): "openai-compatible" | "dedicated" {
  return canonicalProviderKind(kind) === "openai-compatible" ? "openai-compatible" : "dedicated";
}

function jsonArray(value: unknown, field: string, fallback = "[]"): string {
  if (value === undefined) return fallback;
...
function parsedArray(value: string | null): readonly string[] {
...
function urlIsValid(value: string): boolean {
  try {
    new URL(value);
    return true;
  } catch {

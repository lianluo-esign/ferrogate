...
use crate::server::asset_bucket::{AssetBucketClient, AssetBucketConfig};
...
/// to make them pass.
const DEFAULT_BUFFER_LIMIT: u64 = crate::server::assets::INLINE_ASSET_MAX_BYTES;
...
async fn commit_verify_accepts_a_matching_object_without_deleting_it() {
    ...
    let verification = verify_committed_object(
        ...
        &CommitVerificationRequest {
            ...
            admission: crate::server::asset_admission::GatewayBufferBudget::disabled(),
        },
    )
    .await
    .unwrap();
...
impl crate::server::asset_bucket::AssetObjectStore for RecordingStreamStore {
    async fn put_object(
        &self,
        _key: &str,
        _body: &[u8],
    ...
    async fn get_object_stream(
        &self,
        key: &str,
    ) -> anyhow::Result<Option<crate::server::asset_bucket::ObjectByteStream>> {
    ...
    async fn put_object_stream(
        ...
        mut body: crate::server::asset_bucket::ObjectByteStream,
    ) -> anyhow::Result<()> {
    ...
    fn presign_put(
        ...
        _size_bytes: u64,
        _content_sha256_hex: &str,
    ) -> anyhow::Result<crate::server::asset_bucket::PresignedUpload> {
...
fn streamed_request<'a>(
    ...
    streamed_request_with_budget(
        ...
        expected_sha256,
        asset_type,
        crate::server::asset_admission::GatewayBufferBudget::disabled(),
    )
}
...
fn streamed_request_with_budget<'a>(
    ...
    expected_sha256: &'a str,
    asset_type: &'a str,
    admission: &'a crate::server::asset_admission::GatewayBufferBudget,
) -> CommitVerificationRequest<'a> {
    CommitVerificationRequest {
...
async fn the_streaming_commit_leg_consumes_no_buffering_budget() {
    ...
    let _committed = budget
        .admit(
            crate::server::asset_admission::ReadResidency::BufferOnly,
            64 * 1024,
        )
        .await
        .map_err(|_| ())
...
fn commit_request_carries_typed_trust_inputs_for_the_shared_screening_service() {
    ...
    assert!(matches!(
        signature.format,
        crate::server::asset_signature::SignatureFormat::Ed25519
    ));
    assert_eq!(signature.material, "c2ln");
    assert_eq!(signature.key_id.as_deref(), Some("pub-1"));
    assert_eq!(
        commit.publish_visibility(),
        crate::server::asset_publish_gate::PublishVisibility::Public
    );
...
fn commit_request_defaults_trust_inputs_when_omitted() {
    ...
    assert_eq!(
        commit.publish_visibility(),
        crate::server::asset_publish_gate::PublishVisibility::TenantPrivate
    );

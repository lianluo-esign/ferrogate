...
/// #553 stage 2 moved these eight out of the old `gateway/` directory to the
/// crate root, and re-exported them here under their old names so the ~30
/// sibling modules that reach them as `super::body` / `crate::server::body`
/// were untouched. Stage 3b brought those siblings into the same crate, so the
/// re-export is now purely a `crate::` alias -- kept, still, because deleting
/// it would mean rewriting ~30 call sites inside a move-only slice. Collapsing
/// it is a one-line-per-caller follow-up, not this commit's business.
pub(crate) use crate::{
...
pub fn serve(config: Config, source_path: Option<PathBuf>, upgrade: bool) -> AnyResult<()> {

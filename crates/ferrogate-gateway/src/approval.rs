...
/// impl lives here — next to the source vocabulary — because ferrogate-gateway
/// depends on ferrogate-runtime, never the reverse. This is also why the
/// `ApprovalStatus` vocabulary could not be extracted ahead of the #553 stage
/// 3b trunk: the impl is orphan-rule-legal only where the enum is local, so the
/// type and the mapping had to travel together.
///

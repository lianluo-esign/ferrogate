...
impl AppState {
    ...
    pub(crate) async fn read_asset_content(
        ...
        id: &str,
        request_id: &str,
    ) -> Result<(StoredAsset, crate::server::asset_admission::BufferedObject), AssetReadError> {
        ...
        let content = if let Some(storage_uri) = asset.storage_uri.as_deref() {
            ...
            if asset.size_bytes > buffer_limit {
                return Err(AssetReadError::TooLarge(
                    crate::server::asset_bucket::asset_too_large_for_buffering_message(
                        &asset.asset_type,
                        &asset.name,
                        &asset.version,
                        asset.size_bytes,
            ...
            match crate::server::asset_bucket::read_object_bounded(
                &bucket,
                storage_uri,
                asset.size_bytes,
                buffer_limit,
                ...
                crate::server::asset_admission::ReadResidency::InlinedInJsonResponse,
                id,
                request_id,
            )
            .await
            {
                Ok(content) => content,
                Err(crate::server::asset_bucket::BufferedReadRefusal::TooLarge {
                    size_bytes,
                    limit_bytes,
                }) => {
                    return Err(AssetReadError::TooLarge(
                ...
                Err(crate::server::asset_bucket::BufferedReadRefusal::Overloaded {
                    requested_bytes,
                    budget_bytes,
                    waited_ms,
                }) => {
                    return Err(AssetReadError::Overloaded(
                        crate::server::asset_bucket::gateway_buffer_budget_exhausted_message(
                            &asset.asset_type,
                            &asset.name,
                            &asset.version,
                            requested_bytes,
                ...
                Err(crate::server::asset_bucket::BufferedReadRefusal::Transport) => {
                    return Err(AssetReadError::BucketUnavailable(
                        crate::server::asset_bucket::BUCKET_READ_UNAVAILABLE_MESSAGE.to_string(),
                    ))
                }
            }
        } else {
            ...
            crate::server::asset_admission::BufferedObject::unbudgeted(asset.content.clone())
        };
    ...
    /// [`read_object_bounded`](crate::server::asset_bucket::read_object_bounded),
    ...
    pub(crate) fn asset_max_gateway_buffer_bytes(&self) -> u64 {
        self.config
            ...
            .unwrap_or(crate::server::assets::INLINE_ASSET_MAX_BYTES)
    }
    ...
    pub(crate) fn asset_total_gateway_buffer_bytes(&self) -> u64 {
        use crate::server::asset_admission::DEFAULT_CONCURRENT_BUFFERED_READS;
    ...
    pub(crate) fn asset_buffer_admission_wait(&self) -> std::time::Duration {
        std::time::Duration::from_millis(
            self.config
                ...
                .unwrap_or(crate::server::asset_admission::DEFAULT_ADMISSION_WAIT_MS),
        )
    }
    ...
    pub(crate) fn asset_buffer_admission(
        &self,
    ) -> &'static crate::server::asset_admission::GatewayBufferBudget {
        use crate::server::asset_admission::GatewayBufferBudget;
    ...
    pub(crate) fn asset_bucket_client(
        ...
        match bucket.backend {
            ferrogate_config::AssetBucketBackend::S3 => {
                ...
                Some(Box::new(
                    crate::server::asset_bucket::AssetBucketClient::new(
                        crate::server::asset_bucket::AssetBucketConfig {
                            endpoint,
                            bucket: bucket_name,
                            region,
                            access_key_id,

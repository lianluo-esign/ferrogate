...
impl AppState {
    ...
    ///
    /// Async and awaited directly by its callers as of issue #373: its only
    /// hot-path caller, `auth::finalize_auth`, is now `async` (its whole
    /// `authenticate`/`finalize_auth` chain was converted), so this no longer
    /// needs the `block_on_sync_bridge` that parked a worker thread per
    /// request. Its point-lookup reads (`get_quota_policy` per non-empty scope
    /// plus one `resolve_tenant_plan`) are `.await`ed inline.
    pub(crate) async fn resolve_effective_quota(
        &self,
    ...
    /// the #188/#397 defect class). This lives in `ferrogate-gateway` on
    /// purpose: `ferrogate-runtime` deliberately does NOT depend on
    /// `ferrogate-policy` (see the note on [`AgentBudgetPolicy`]), and this is
    /// the crate that already owns both the quota resolution and the runtime
    /// types.
    ///
    /// Returns:
            ...
            .filter(|policy| policy.revision.scope.matches(selection))
        {
                ...
                && policy.revision.streaming == PolicyStreamingMode::RejectStreaming
            {
                ...
                let candidate = (!shadow).then(|| GuardrailMatch {
                    ...
                    action_kind: GuardrailActionKind::Block,
                    segment_id: None,
                    byte_start: None,
                    byte_end: None,

...
// vocabulary, the credential primitives, the external auth-service client and
// the request-admission pipeline (issue #553 stages 3b-0 and 3b).

//! What a caller IS, what it may DO, and how a presented credential becomes an
//! admitted request.
//!
//! # One module again
//!
//! Stage 3b-0 of issue #553 split `ferrogate-cli`'s `auth.rs` in half and left
//! a note here saying where the other half would land. This is that landing.
//! `AppState` moved into this crate with the stage-3b trunk, so the
//! request-admission functions that read it followed, and the workspace has
//! exactly one module named `auth` again. Nothing about either half changed on
//! the way: the merge is a concatenation plus one added `use`.
//!
//! Four groups live here, and the split between them is the natural next one
//! if this file ever needs dividing again:
//!
...
//!    tenant is this caller, and is it root?", and every one of these is a
//!    pure function of the credential.
...
//! 3. **The external auth-service client**, plus the standalone control-api
//!    service's own fail-closed gate ([`authenticate_admin_gate`]), which is
//!    written against plain config values rather than against state.
//! 4. **The request-admission pipeline**: [`authenticate`],
//!    [`authenticate_for_lifecycle_recovery`], `finalize_auth`,
//!    `authenticate_durable`, `require_request_budget`,
//!    [`authorize_scoped_resource`] and
//!    [`authorize_self_hosted_worker_scope`]. Between them these reach twelve
//!    distinct [`AppState`] accessors -- the credential sources
//!    (`auth_required`, `config.auth_service`, `config.tenancy`,
//!    `config.api_keys`, `durable_api_key_authenticator`), the governance
//!    calls (`require_usable_tenancy`, `resolve_effective_quota`,
...
//!    `self_hosted_worker_record`). That is why it could not travel ahead of
//!    the trunk, and why no host trait was invented to let it: an abstraction
//!    with exactly one implementor, carrying dynamic dispatch on the
//!    per-request authentication path, that this stage would have deleted.
//!
//! # Two test files, one module
//!
//! `auth_test` holds the cases that need nothing but a credential;
//! `auth_admission_test` holds the ones that build a real `AppState`. They
//! stayed separate FILES under this one module rather than being concatenated,
//! because each defines fixture helpers under names the other also uses --
//! merging them would have meant renaming test helpers inside a move-only
//! slice, which is exactly the kind of edit that hides a real change.
...
pub(crate) fn api_key_is_expired(key: &ApiKey, now_unix_seconds: u64) -> bool {
    key.expires_at_unix

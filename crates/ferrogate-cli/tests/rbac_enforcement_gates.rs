...
// Created: 2026-07-07
// description: Proves the two RBAC enforcement points wired this round
// (issue #182's continuation: retrofitting other business modules, per
// #168's originally-scoped-but-never-wired mcp_enabled/
// self_hosted_workers_enabled) actually gate access -- not just that they
...
fn cleanup(dsn: &str, tenant_id: &str) {
    ...
    // two tests in this file share the same role-slug namespace and run
    // in the same process, so a leftover binding from the *other* test's

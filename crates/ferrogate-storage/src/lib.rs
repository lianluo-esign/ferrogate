...
// unqualified `*_from_row(...)` call site in this crate resolves unchanged.
...
pub(crate) use postgres_row_mappers::*;
...
impl PostgresControlPlaneStore {
    ...
    async fn upsert_quota_policy(&self, policy: &StoredQuotaPolicy) -> Result<(), StorageError> {
        ...
        let asset_max_object_bytes = policy.asset_max_object_bytes.map(saturating_i64);
    ...
    async fn upsert_plan(&self, plan: &StoredPlan) -> Result<(), StorageError> {
        ...
        let default_asset_max_object_bytes =
            plan.default_asset_max_object_bytes.map(saturating_i64);
            ...
            .execute(
                "INSERT INTO plans \
                 (id, name, slug, mcp_enabled, self_hosted_workers_enabled, \
                  ...
                  default_download_rpm_limit, default_asset_max_object_bytes, \
...
fn default_free_plan() -> StoredPlan {
    StoredPlan {
        ...
        // #259: no dedicated per-object ceiling on the free plan -- individual
        // objects stay bounded by the operator's global presign ceiling and
        // the cumulative quota above, exactly as before this field existed.
        default_asset_max_object_bytes: None,
        // #428: no default agent runtime-cost ceiling on the free plan -- an
        // operator opts a tenant into agent-cost governance explicitly, exactly
        // as with the monthly budget above.
...
pub struct StoredPlan {
    ...
    /// Per-object (not cumulative) default asset byte ceiling (issue #259):
    /// each individual asset object must be <= this. Independent of the
    /// cumulative `default_asset_storage_quota_bytes`; a tenant-scoped
    /// `StoredQuotaPolicy.asset_max_object_bytes` overrides it exactly as
    /// `asset_storage_quota_bytes` overrides the cumulative default. `None`
    /// means "no dedicated per-object default" (the presign path then falls
    /// back to the operator ceiling / cumulative quota alone).
    #[serde(default)]
    pub default_asset_max_object_bytes: Option<u64>,
    /// Per-tenant monthly USD ceiling on CF-hosted-agent runtime cost (issue
    /// #428): the tenant-wide default `resolve_effective_quota` applies as the
    /// floor when no explicit `quota_policies.agent_cost_budget_usd` is set in
    /// the chain. A monetary value that mirrors `default_monthly_budget_usd`
    /// (min-across-the-chain, plan default), NOT the tenant-only byte ceilings.
    /// `None` means no default agent-cost ceiling. The per-tenant source a
    /// later connector resolves into the slice-A `AgentBudgetPolicy.ceiling_usd`.
    #[serde(default)]
    pub default_agent_cost_budget_usd: Option<f64>,
...
pub struct StoredQuotaPolicy {
    ...
    /// (issue #259): a per-object (not cumulative) asset byte ceiling, distinct
    /// from the cumulative `asset_storage_quota_bytes` above. Tenant-owned for
    /// the same reason; `None` means "no override, fall back to the plan
    /// default".
    #[serde(default)]
    pub asset_max_object_bytes: Option<u64>,
    /// Per-tenant monthly USD ceiling on CF-hosted-agent runtime cost (issue
    /// #428): a monetary value that mirrors `monthly_budget_usd` -- settable at
    /// any scope and merged `min`-across-the-chain by `resolve_effective_quota`
    /// (NOT tenant-only like the asset byte ceilings), with the plan's
    /// `default_agent_cost_budget_usd` as the floor. `None` means no agent-cost
    /// ceiling defined at this scope. The per-tenant source a later connector
    /// resolves into the slice-A `AgentBudgetPolicy.ceiling_usd`.
    #[serde(default)]
...
pub fn validate_quota_policy(policy: &StoredQuotaPolicy) -> Result<(), StorageError> {
    ...
    if policy.scope_type != QuotaScopeKind::Tenant && policy.asset_max_object_bytes.is_some() {
        return Err(StorageError::Runtime(
...
impl RuntimeStorageRepositories {
    ...
    pub async fn upsert_admin_user(&self, user: StoredAdminUser) -> Result<(), StorageError> {
        self.control_plane.store().upsert_admin_user(user).await
    }
    ...
    pub async fn get_admin_user_by_id(
        ...
        self.control_plane.store().get_admin_user_by_id(id).await
    }
    ...
    pub async fn upsert_sso_provider_config(
        ...
        self.control_plane
            .store()
            .upsert_sso_provider_config(config)
            .await
    }
    ...
    pub async fn get_sso_provider_config(
        ...
        self.control_plane
            .store()
            .get_sso_provider_config(tenant_id)
            .await
    }
    ...
    pub async fn delete_sso_provider_config(&self, tenant_id: &str) -> Result<bool, StorageError> {
        self.control_plane
            .store()
            .delete_sso_provider_config(tenant_id)
            .await
    }
    ...
    pub async fn take_sso_pending_flow(
        ...
        self.control_plane
            .store()
            .take_sso_pending_flow(state, now_unix)
            .await
    }
    ...
    pub async fn upsert_admin_user_refresh_token(
        ...
        self.control_plane
            .store()
            .upsert_admin_user_refresh_token(token)
            .await
    }
    ...
    pub async fn revoke_admin_user_refresh_tokens_for_tenant(
        ...
        self.control_plane
            .store()
            .revoke_admin_user_refresh_tokens_for_tenant(user_id, tenant_id, revoked_at_unix)
            .await
    }
    ...
    pub async fn upsert_quota_policy(&self, policy: StoredQuotaPolicy) -> Result<(), StorageError> {
        ...
        self.control_plane.store().upsert_quota_policy(policy).await
    }
    ...
    pub async fn list_quota_policies(&self) -> Result<Vec<StoredQuotaPolicy>, StorageError> {
        self.control_plane.store().list_quota_policies().await
    }
    ...
    pub async fn upsert_asset(&self, asset: StoredAsset) -> Result<(), StorageError> {
        self.control_plane.store().upsert_asset(asset).await
    }
    ...
    pub async fn get_asset(&self, id: &str) -> Result<Option<StoredAsset>, StorageError> {
        self.control_plane.store().get_asset(id).await
    }
    ...
    pub async fn list_assets(
        ...
        self.control_plane
            .store()
            .list_assets(tenant_id, asset_type)
            .await
    }
    ...
    pub async fn delete_asset(&self, id: &str) -> Result<bool, StorageError> {
        self.control_plane.store().delete_asset(id).await
    }
    ...
    pub async fn delete_asset_channel(&self, id: &str) -> Result<bool, StorageError> {
        self.control_plane.store().delete_asset_channel(id).await
    }
    ...
    pub async fn list_all_assets(&self) -> Result<Vec<StoredAsset>, StorageError> {
        self.control_plane.store().list_all_assets().await
    }
    ...
    pub async fn list_all_asset_channels(&self) -> Result<Vec<StoredAssetChannel>, StorageError> {
        self.control_plane.store().list_all_asset_channels().await
    }
    ...
    pub async fn billing_ledger_entry(
        ...
        self.control_plane.store().billing_ledger_entry(id).await
    }
    ...
    pub async fn list_due_billing_reports(
        ...
        self.control_plane
            .store()
            .list_due_billing_reports(now_unix, limit)
            .await
    }
    ...
    pub async fn dead_letter_billing_report(
        ...
        self.control_plane
            .store()
            .dead_letter_billing_report(id, dead_lettered_at_unix)
            .await
    }
    ...
    pub async fn list_dead_lettered_billing_reports(
        ...
        self.control_plane
            .store()
            .list_dead_lettered_billing_reports(limit)
            .await
    }
...
mod tests {
    ...
    fn quota_policy_upsert_get_list_and_delete_roundtrip() {
        ...
        block_on(repositories.upsert_quota_policy(StoredQuotaPolicy {
            ...
            agent_cost_budget_usd: Some(90.0),
        ...
        assert_eq!(tenant_policy.asset_max_object_bytes, Some(1_048_576));
        assert_eq!(tenant_policy.agent_cost_budget_usd, Some(90.0));
        assert_eq!(

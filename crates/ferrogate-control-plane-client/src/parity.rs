...
pub const REVIEWED_EXCLUSIONS: &[ReviewedExclusion] = &[
    // The three x402 spend-policy diagnostics (#351) that used to sit here are
    // now bound by the `x402-spend-policies` group in `organization.rs`: their
    // exclusion deferred them to the #361 policy family, and #361 is the slice
    // that lands it, so leaving the rows would have made the exclusion a rubber
    // stamp for its own owner.

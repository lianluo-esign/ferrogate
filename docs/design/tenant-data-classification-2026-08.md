...
| C4 | `tenant_databases` | platform-shared | the storage registry; Durable Object routing keeps the roster and provisioning state, drops `database_uuid`/`database_name`, and retains `binding_name` only for explicit native-binding compatibility; `location_hint` records object residency |

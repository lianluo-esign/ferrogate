...
| **L5** — `r2BucketNameForTenant` length-prefixes with UTF-16 code units | DEVIATION, LOW | **STILL OPEN.** `r2.ts:293` is `${DOMAIN}:${tenant.length}:${tenant}`; Rust's `len` is UTF-8 **bytes**. Diverges for any non-ASCII tenant id; the golden vectors are all ASCII. LOW only because nothing consumes tenant R2 provisioning on either side — **MEDIUM the moment it is wired**, because a bucket name IS the isolation boundary. One line: `new TextEncoder().encode(tenant).length` |

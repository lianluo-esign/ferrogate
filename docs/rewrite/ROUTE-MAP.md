# FerroGate — route map (278 operations → Hono apps)

Derived from the authoritative contract `docs/openapi/runtime-api-contract.json`
(`version: 1`, 278 operations, 41 route groups). **That JSON is the source of
...
| App | Ops | Surface |
|---|---:|---|
| `apps/control-plane` | **215** | `/admin/v1/**` (210) + `/admin`, `/admin/`, `/admin/dashboard`, `/admin/status`, `/metrics` |
...
**Auth kinds** (278): `bearer` 264 · `internal` 6 · `anonymous` 7 · `method_dependent` 1.
**Visibility**: `admin` 211 · `public` 60 · `internal` 7.
**Methods**: GET 127 · POST 86 · DELETE 29 · PUT 20 · PATCH 16.
...
>
> `apps/control-plane` went 197 → 215 (197 + 3 + 3 + 6 + 2 + 4) and `apps/gateway`
...
1. **Every operation carries `visibility`, `auth.kind`, `auth.scope`, and
   ...
   hand-written per-route guards — one table-driven middleware keeps all 278 in sync.
...
## Dynamic surfaces (NOT in the 278)
...
- Generate the route table from the JSON at build time (or import it directly)
  ...
  asserting **`routes.length === 278`** and that every contract `operation_id`

# FerroGate — route map (275 operations → Hono apps)

Derived from the authoritative contract `docs/openapi/runtime-api-contract.json`
(`version: 1`, 275 operations, 41 route groups). **That JSON is the source of
...
| App | Ops | Surface |
|---|---:|---|
| `apps/control-plane` | **214** | `/admin/v1/**` (209) + `/admin`, `/admin/`, `/admin/dashboard`, `/admin/status`, `/metrics` |
...
**Auth kinds** (275): `bearer` 261 · `internal` 6 · `anonymous` 7 · `method_dependent` 1.
...
1. **Every operation carries `visibility`, `auth.kind`, `auth.scope`, and
   ...
   hand-written per-route guards — one table-driven middleware keeps all 275 in sync.
...
## Dynamic surfaces (NOT in the 275)
...
- Generate the route table from the JSON at build time (or import it directly)
  ...
  asserting **`routes.length === 275`** and that every contract `operation_id`

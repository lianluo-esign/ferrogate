...
| ID | Seam | Mutation | Confirm | Expected RED | Channel | Tier |
|---|---|---|---|---|---|---|
| MCP-U1 | **NEW, #765.** The IDLE REAPER: `await this.#arm(nowMs)` in `open`/`reconcile`/`recordUpstreamHealth`/`append` (5 call sites) plus `await this.close()` inside `override async alarm()`. Without the arming no alarm is ever scheduled and nothing evicts; without the `close()` the alarm fires and the session survives | `MUT-2` neutralise all five arms: `"await this.#arm("→"/*MUTATION-765-ARM*/ void ("`; **and separately** `MUT-1` the `await this.close();` in `alarm()` | `grep -c 'MUTATION-765-ARM' src/unified.ts` → **5**; `grep -c 'MUTATION-765-CLOSE'` → 1 | `test/unified-session-eviction.test.ts` — **7 RED of 7** on the arming mutation (every one at `runDurableObjectAlarm` returning `false`), **5 RED** on the `close()` deletion (the mount test at `expected 200 to be 400`: the resume is served as if the session were live). The refusal is TWO layers — the ingress answers off `store.status`, the object's own `replay` refuses with the same reason — so each has its own gate: dropping the ingress branch alone is **1 RED**, dropping `replay`'s tombstone branch alone is **1 RED**, dropping both is **2 RED** | DEF | T1 |

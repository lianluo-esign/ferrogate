...
| **`ferrogate-cloudflare` — the 21st crate** (§6 item 5; "the single strongest argument against deleting the Rust") | `packages/cloudflare` — mounted account-management client plus scopes/envelope/errors; the unmounted per-tenant R2 provisioning slices were retired by #744 | Narrowing `RETRYABLE_STATUSES` to `[429]` ⇒ **5 RED** across `test/retry.test.ts` + `test/client.test.ts`. **Wired, not merely written:** `packages/storage/src/tenant-rest.ts` now routes the D1 REST transport through `executeWithRetry`; forcing `isRetryableOutcome` to `false` ⇒ **5 RED** in `packages/storage/test/d1/rest-retry.test.ts`. #744 records the shared-bucket-plus-tenant-prefix decision |
...
| **§4.1 — everything only a real deploy can settle** | Unchanged for the remaining Cloudflare API slices. The former per-tenant R2 provisioning slices were removed by #744; the deployed asset path uses one shared bucket and tenant key-prefix isolation |
...
3. **Re-audit the two escalation-config apps for the harness trap.** The
   ...
   checked for it.
4. **Decide the remaining deferred sub-question the code now names**: whether the
   ...
   bucket-per-tenant. #744 resolved that question for the deployed asset path:
   it uses one shared bucket with tenant key-prefix isolation.

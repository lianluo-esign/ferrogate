...
| B2 | **R2 is not enabled on the live account** (prior finding) but `apps/gateway` declares `[[r2_buckets]] ASSETS` | `apps/gateway/wrangler.toml` | Either enable R2 on the account and create the bucket, or delete the `[[r2_buckets]]` stanza for this one verification. With no bucket the gateway still boots and the 18 asset ops answer `503 asset_bucket_unavailable` — an acceptable posture for the verification, but a **declared** bucket that does not exist fails the deploy outright. |

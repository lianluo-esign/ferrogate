...
| B4 | **CLOSED by #859.** `apps/agent-runtime` now declares live tenant `DB` and control `CONTROL_DB` authorities; `[env.unit]` intentionally omits non-inheritable D1 bindings for the default unit suite | `apps/agent-runtime/wrangler.toml`, `apps/agent-runtime/vitest.config.ts` | At deploy, replace both placeholder database IDs and apply `sql/d1-ts/tenant` plus `sql/d1-ts/control`. The durable harness binds and migrates both schemas; the default unit project selects `environment = "unit"` so its explicit fixtures are not shadowed by empty unmigrated D1 databases. Keep `DB` and `CONTROL_DB` together: the authenticated surfaces and durable control/evidence paths require both. |
...
The deploy file carries two D1 placeholders: tenant `DB` and control
`CONTROL_DB`. Replace both database IDs and apply the matching migrations before
the authenticated surface is verified. The default unit project selects the
`unit` Wrangler environment, which intentionally has no D1 bindings; the
durable harness binds and migrates both authorities instead. See **B4**.

| Var | Committed placeholder | Value to supply |
|---|---|---|
| `AGENT_UPSTREAMS` | `"[]"` | **Wave 21 (cutover CLASS A item A3, leg 2).** The operator catalog of A2A upstreams (Rust `[[agent_upstreams]]`) behind the DISPATCH path `POST /v1/agents/{name}` and the `message:*` verbs. When `CONTROL_DB` is bound, the `control_plane_resources` documents of kind `agent-upstreams` — the same rows `apps/gateway` publishes `GET /.well-known/agent.json` from and the same rows `DELETE /admin/v1/agent-upstreams/{id}` removes — become the whole registry, read once per dispatch with no cache, and **the var is NOT merged over them** (`src/agents/registry.ts`). When the control database is unavailable, dispatch fails closed rather than silently treating this var as an authoritative fleet catalog. Verify the shared withdrawal with **V-A3** below. |
| `AGENT_WORKFLOWS` | `"[]"` | **Wave 20 (cutover HOLD item A2).** The operator workflow table (Rust `[[agent_workflows]]`) read by the tool-side graph gate in `src/runs/workflow.ts`. Supply a JSON array of workflow documents to pin tools to nodes and constrain edge transitions; leave `"[]"` if workflows are managed exclusively through the admin API, which writes `control_plane_resources` documents of kind `agent-workflows` that this var is materialised OVER. **`"[]"` is not "allow everything":** the gate is opt-in by the `x-ferrogate-workflow-id` header, so a caller that declares a workflow against an empty table is refused `400 workflow_not_found`, and a caller that declares none is unaffected. Reading the durable half requires the live `CONTROL_DB` declared above; an unavailable control database fails closed rather than promoting this var to a fleet-wide authority. |
...
| `DB` / `CONTROL_DB` on agent-runtime | agent-runtime | **Declared with deploy-time placeholders** (B4). Replace both IDs and migrate both schemas before verification. These handles are also how this Worker reads the operator drain, tenant lifecycle authority and control evidence mirror; an incomplete binding is an incomplete fleet deployment. See **B11**. |
...
9. agent-runtime: replace the two `database_id` placeholders and apply both
   D1 migration trees (B4 — `CONTROL_DB` and `DB` together; see §7.2, the
   half-bound posture is the silent one). Set `FG_DEV_IN_MEMORY_PORTS = "0"`
   for the deploy (B1). Leave
...
### 7.2 B1 + B4 — the HALF-BOUND posture remains dangerous

B4 is now closed in the committed deploy config: `apps/agent-runtime` declares
both D1 authorities, and the durable harness binds and migrates both. The named
`unit` environment intentionally has no D1 bindings so the default suite keeps
its explicit fixtures. The deploy invariant is still that `DB` and
`CONTROL_DB` are supplied together.

**A partially edited deployment is still dangerous.** Bind `DB`, forget
`CONTROL_DB`, and leave the committed `FG_DEV_IN_MEMORY_PORTS = "1"` (B1) in
place: `resolveDeps` **succeeds** — `apiKeys` is durable, `workerIdentities`

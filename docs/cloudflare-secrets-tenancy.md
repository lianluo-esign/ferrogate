...
| **Per-tenant** credentials (tenant provider keys the gateway holds, per-user MCP identities, tenant webhook secrets) | **Existing backends, unchanged**: the Postgres at-rest AEAD path (XChaCha20-Poly1305, e.g. per-user MCP identity in `crates/ferrogate-gateway/src/state_mcp_identity.rs`, master-keyed by `FERROGATE_MCP_IDENTITY_KEY`), or `vault://` (HashiCorp Vault KV v2 seam in `crates/ferrogate-secrets`) | Unbounded cardinality; needs load-time *reads* (impossible over CF REST); needs tenant-scoped isolation and instant revocation without a Worker redeploy |

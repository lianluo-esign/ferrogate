...
The TypeScript contract types, the Python operation catalog, and the
admin-console contract types are generated from
`docs/openapi/admin-api.openapi.json` and committed. The vendored SDK package
entry points are `sdks/typescript/` and `sdks/python/`; every generated
artifact is listed in `tools/generated-clients/artifacts.mjs` and regenerated
by ONE command:
...
Build and inspect the vendored package artifacts without publishing them:

```bash
bun run check:sdk-packages
```

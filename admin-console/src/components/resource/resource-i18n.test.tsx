...
describe("ResourceTable chrome renders per locale", () => {
  ...
  it("localizes the row-action menu labels in Simplified Chinese", async () => {
    ...
    expect(
      await screen.findByRole("menuitem", { name: zhCN["resource.action.edit"] }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("menuitem", { name: zhCN["resource.action.delete"] }),
    ).toBeInTheDocument();
  });
});
...
describe("ResourceForm chrome renders per locale", () => {
  it("shows Simplified Chinese Cancel chrome and localized validation copy", async () => {
    ...
    expect(
      screen.getByRole("button", { name: zhCN["resource.action.cancel"] }),
    ).toBeInTheDocument();
...
describe("ResourcePage chrome renders per locale", () => {
  ...
  it("renders English pagination + create chrome, data-driven title untouched", async () => {
    ...
    expect(
      screen.getByRole("button", { name: en["resource.pagination.previous"] }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: en["resource.pagination.next"] }),
    ).toBeInTheDocument();

...
describe("LoginPage", () => {
  it.each(CASES)("renders every visible string from the %s catalog", (locale) => {
    ...
    expect(screen.getByText(translate(locale, "auth.login.subtitle"))).toBeInTheDocument();
    ...
    expect(screen.getByText(translate(locale, "auth.login.registerPrompt"))).toBeInTheDocument();
    expect(
  ...
  it("uses the localized fallback error copy per locale", () => {
    ...
    expect(translate("zh-CN", "auth.login.error")).not.toBe(translate("en", "auth.login.error"));
  });
});

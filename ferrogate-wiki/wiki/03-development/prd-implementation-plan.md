...
| P6 | 可观测性、请求日志、Storage 与计费事件 | 进行中 | 55% | 已定义 Token usage、模型价格、成本估算、Billing Event、请求日志、usage aggregate、in-memory repository/sink 和观测 span 模板边界；非流式 AI Proxy 成功响应已写入 in-memory Billing Event；OpenTelemetry exporter、request log 运行路径和 body 记录策略待后续切片 |

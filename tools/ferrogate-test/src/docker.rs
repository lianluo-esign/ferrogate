...
use crate::{
    ...
    cli::{DockerScenario, IMAGE_TAG},
    fixtures::{
        analytics_direct_clickhouse_gateway_config, analytics_gateway_config, gateway_config,
        guardrail_complete_gateway_config, guardrail_gateway_config,
        guardrail_response_gateway_config, redis_counter_gateway_config, vector_clickhouse_config,
    },
    http::http_request,
};
...
pub(crate) const POSTGRES_CONTAINER: &str = "ferrogate-e2e-postgres";
pub(crate) const POSTGRES_MIGRATION_SOURCE_CONTAINER: &str =
    "ferrogate-e2e-postgres-migration-source";
...
pub(crate) fn run_docker_scenario(scenario: DockerScenario, image: &str) -> Result<()> {
    match scenario {
...
pub(crate) fn run_all_docker_scenarios(image: &str) -> Result<()> {
...
pub(crate) fn docker_args<I, S>(args: I) -> Result<()>
where

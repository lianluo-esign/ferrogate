...
describe("PLATFORM LIMIT — no runtime bind-by-uuid for D1 (§1.7)", () => {
   ...
   */
  test("the REST strategy is declared as lacking BOTH money-path primitives", () => {
  ...
  test("the REST router THROWS rather than serving a non-atomic write path", async () => {
    ...
    // the more dangerous artifact; the refusal must name the missing primitives.
    expect((error as StorageError).message).toContain("RETURNING");
    expect((error as StorageError).message).toContain("batch()");
  });
});

 ...
 *    comment lines containing a `;` mid-prose, and the TWELVE files have 31
 *    between them (18 in 0001, 1 in 0003, 5 in 0005, 3 in 0008, 2 in 0009,
 *    1 in 0011 and 1 in 0012) — 31, not 18,
 ...
 *    the table.
 *  * It is safe today because, measured over all TWELVE files, every non-comment
...
export class TenantDataObject extends DurableObject {
  ...
  constructor(ctx: DurableObjectState, env: unknown) {
    ...
    // gate inside `#migrate` is what keeps that from re-applying 98 statements
   ...
   * re-running the 98 statements of the twelve files — 34 `CREATE TABLE IF NOT
   * EXISTS`, 42 `CREATE INDEX`, 5 `CREATE UNIQUE INDEX`, 10 `ALTER TABLE … ADD
   ...
   * whole census then went stale again the moment `0012_request_logs_agent_runs.sql`
